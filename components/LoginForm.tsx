...
export default function LoginForm() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      // Always navigate immediately — never block with a full-screen overlay.
      // Biometric setup prompt appears as inline banner after login.
      const bioStatus = localStorage.getItem(BIOMETRIC_KEY)
      if (webAuthnSupported && !bioStatus) {
        // Mark as "pending setup" so the inline banner shows on community page
        localStorage.setItem(BIOMETRIC_KEY, 'pending')
      }
  ...
  return (
    <>
      ...
      {/* Biometric prompt moved to inline banner on community page */}
    </>
  )
}
